...
class PricingEnvironment:
    def __init__(
        ...
        self.demands = jrng.normal(key, shape=(num_consumers,)) * demand_std + demand_mean
        # For simplicity, initialize each consumer's estimate of cost (if needed) to a prior, e.g., demand_mean/2
    ...
    def step(self, producer_prices):
        ...
        sales = producer_prices <= self.demands
        key = jrng.PRNGKey(self.seed)
        ...
        consumer_gains = jnp.where(sales, self.demands - producer_prices, 0.0)
        ...
        noise = jrng.normal(key, shape=(self.num_consumers,)) * self.lie_std
    ...
    def reset(self):
        ...
        if self.communication_mode == 'cost':
            self.cost_estimates = jnp.ones(self.num_consumers) * (jnp.mean(self.demands) / 2)
...
def run_simulation(seed = 1234):
    ...
    for round_idx in range(num_rounds):
        ...
        exploration_noise = jrng.normal(key, shape=(env.num_consumers,)) * 0.5
        producer_prices = 0.8 * env.demands + exploration_noise
